...
export function useWorkersMapPoints(filters: WorkersMapFilters): State<WorkerMapPoint> & { refetch: () => void } {
  return useMapPoints<WorkerMapPoint>('workers', filters);
}

export function usePatientsMapPoints(filters: PatientsMapFilters): State<PatientMapPoint> & { refetch: () => void } {
  return useMapPoints<PatientMapPoint>('patients', filters);
}

function useMapPoints<P>(kind: MapKind, filters: WorkersMapFilters | PatientsMapFilters): State<P> & { refetch: () => void } {
  const [state, setState] = useState<State<P>>({ ...EMPTY, isLoading: true, error: null });
