 ...
 *
 * Escopo sempre presente (lex C3): raio a partir de um centro, que nasce em
 * CABA e muda por clique no mapa ou escolhendo um paciente. Sem geocoding,
 * sem endereço digitado (lex C7). País é filtro de primeira classe (C4).
 */
...
import {
  DEFAULT_CENTER, DEFAULT_COUNTRY, DEFAULT_RADIUS_KM, PATIENT_PICKER_RADIUS_KM, PATIENT_STATUSES, PATIENT_STATUS_COLOR,
...
export function AdminMapPage(): JSX.Element {
  ...
  // Seletor "centrar en paciente" (aba de prestadores): pacientes num raio largo
  // em volta do centro atual — escopo obrigatório, então nunca "todos".
  const pickerFilters = useMemo<PatientsMapFilters>(() => ({ country, center: DEFAULT_CENTER, radius_km: PATIENT_PICKER_RADIUS_KM }), [country]);

  const workers = useWorkersMapPoints(workersFilters);
  const patients = usePatientsMapPoints(patientsFilters);
  const picker = usePatientsMapPoints(pickerFilters);
  ...
  const countryOptions: SelectOption[] = [{ value: 'AR', label: 'Argentina' }, { value: 'BR', label: 'Brasil' }];
  ...
  const patientPickerOptions: SelectOption[] = [
    ...
    ...picker.points.filter((p) => p.lat !== null).map((p) => ({ value: p.addressId ?? p.id, label: `${p.name}${placeLabel(p) ? ` · ${placeLabel(p)}` : ''}` })),
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-4">
        <aside className="flex flex-col gap-3 min-w-0">
          <div className="grid grid-cols-2 gap-2">
            <Select data-testid="map-country" options={countryOptions} value={country} onValueChange={(v) => setCountry(v as MapCountry)} aria-label={t('admin.map.country', 'País')} />
          ...
          {kind === 'workers' ? (
            <>
              <Select data-testid="map-center-patient" options={patientPickerOptions} value={centerPatientId} onValueChange={onPickPatient} aria-label={t('admin.map.centerOnPatient.label', 'Centrar en paciente')} />
          ...
          <ul className="divide-y divide-gray-100 border border-gray-200 rounded-md max-h-[440px] overflow-y-auto" data-testid="map-list">
            {kind === 'workers'
              ? workers.points.map((p) => (
                <li key={p.id} data-testid="map-list-item" data-point-id={p.id} data-has-coords={p.lat !== null} className={`px-3 py-2 cursor-pointer ${selectedId === p.id ? 'bg-blue-50' : 'hover:bg-gray-50'}`} onClick={() => setSelectedId(p.id)}>
                  <div className="flex items-center gap-2">
                    ...
                    <Link to={`/admin/workers/${p.id}`} className="hover:underline truncate" onClick={(e) => e.stopPropagation()}><Text as="span" size="sm" weight="medium" color="primary">{p.name}</Text></Link>
              ...
              : patients.points.map((p) => (
                <li key={p.addressId ?? p.id} data-testid="map-list-item" data-point-id={p.id} data-has-coords={p.lat !== null} className={`px-3 py-2 cursor-pointer ${selectedId === (p.addressId ?? p.id) ? 'bg-blue-50' : 'hover:bg-gray-50'}`} onClick={() => setSelectedId(p.addressId ?? p.id)}>
                  <div className="flex items-center gap-2">
                    <span className="inline-block w-2.5 h-2.5 rounded-full shrink-0" style={{ background: PATIENT_STATUS_COLOR[p.status] ?? '#6b7280' }} />
                    <Link to={`/admin/patients/${p.id}`} className="hover:underline truncate" onClick={(e) => e.stopPropagation()}><Text as="span" size="sm" weight="medium" color="primary">{p.name}</Text></Link>
                    {p.distanceKm !== null && <Text as="span" size="xs" color="secondary" className="ml-auto shrink-0">{distanceLabel(p.distanceKm)}</Text>}
                  </div>
                  <Text as="div" size="xs" color="secondary" className="truncate">
