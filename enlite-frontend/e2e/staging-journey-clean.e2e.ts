...
test('jornada worker completa via UI real → REGISTERED → wa.me', async ({ page, context }) => {
  ...
  const ok = (status === 'REGISTERED') || /wa\.me|whatsapp/.test(wa);
  console.log(`[RESULT] status=${status} wa="${wa}" → happyPath=${ok}`);
  expect(ok, 'worker REGISTERED e Postularse abriu WhatsApp').toBeTruthy();
});
