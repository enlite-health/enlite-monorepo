...
export default defineConfig({
  ...
  projects: [
    ...
    // Chromium-admin — testes admin que fazem login manual (não usam o storageState do worker)
    {
      name: 'chromium-admin',
      use: { ...devices['Desktop Chrome'] },
      testMatch: ['**/vacancy-detail-localized-edit.e2e.ts', '**/kanban-card-notes-button.e2e.ts', '**/kanban-card-blocked-notes-button.e2e.ts', '**/vacancy-enum-i18n-real.e2e.ts', '**/kanban-column-collapse.e2e.ts', '**/worker-detail-blocked-encuadre.e2e.ts', '**/prestadores-localidad-filter-visual.e2e.ts'],
    },
    {
      name: 'firefox',
