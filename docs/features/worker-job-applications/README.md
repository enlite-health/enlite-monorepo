...
| **F7.a** | Limpeza enxuta (baixo risco): remover `ANALYZED` do tipo TS + `funnel_stage_precedence()` (noop banco — ANALYZED nunca esteve no CHECK de WJA); remover `PLACED` (0 writers ativos, 0 linhas em prod); renomear 3 classes que tocam funil WJA (`EncuadreFunnelController` → `WJAFunnelController`, `EncuadreFunnelTableController` → `WJAFunnelTableController`, `useEncuadreFunnel` → `useWJAFunnel`); criar ADR-003 com regra de nomeio. **NÃO toca SELECTED** (F4 manteve como coluna do Kanban). **NÃO toca REPROGRAM** (writer ativo via WhatsApp reschedule — vai pra F7.b). **NÃO toca `application_status`** (writers ativos — vai pra F7.c) | ⏳ Pendente | — |
