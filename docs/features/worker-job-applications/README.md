...
| **F5** | UNIQUE `(worker_id, job_posting_id)` em `encuadres` + consolidar ~20k duplicatas históricas (richness score + recência) + 6 call sites atualizados de `ON CONFLICT (dedup_hash)` para par composto + trigger 189 atualizado. REPROGRAMAR já edita WJA sem criar encuadre novo (descoberto no Explore — desnecessário mexer no use case) | ✅ Concluída 2026-05-24 | TBD (commit) |
