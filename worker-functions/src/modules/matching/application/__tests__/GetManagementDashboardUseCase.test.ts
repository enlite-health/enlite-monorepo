...
describe('GetManagementDashboardUseCase', () => {
  ...
  it('agrega big numbers, equipe armada, pacientes, horas, prioridades, funil, encuadres e cadastros', async () => {
    const db = mockDb(
      ...
      8, // encuadres agendados esta semana: NUMERADOR do % de capacidade (default 36 → 22,2%)
    ...
    expect(result).toEqual({
      ...
      encuadres: {
        ...
        semDataRegistrada: 0,
        // Capacidade default 36 (env ausente, pedido time recrutamento 22/09): 8/36 = 22,2%.
        pctCapacidadeSemana: { agendados: 8, capacidade: 36, pct: 22.2 },
      },
      cadastros: {
        leads: 6882,
        completos: 250,
