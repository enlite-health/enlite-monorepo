...
function makeApp(): { app: express.Express; calls: Record<string, jest.Mock> } {
  ...
  app.use('/api/admin', createFunnelStageMessagesRoutes(calls as unknown as FunnelStageMessagesController, authMiddleware));
  return { app, calls };
}
...
describe('createFunnelStageMessagesRoutes', () => {
  ...
  it('PUT /funnel-stage-messages/:stage → controller.update sob requireAdmin (quem configura ≠ quem dispara)', async () => {
    const { app, calls } = makeApp();
    ...
    expect(seen).toEqual(['admin PUT /funnel-stage-messages/COMPLETED']);
  });
