...
describe('workerContainerAccess — a célula de cada container da ficha do prestador (D286 fase 2)', () => {
  it('4 containers, cada um com o recurso da C3/F2 — nenhum recurso novo nasce aqui', () => {
    expect(WORKER_CONTAINERS).toEqual(['contact', 'dossier', 'documents', 'encuadres']);
    expect(WORKER_CONTAINERS.map(workerContainerCell)).toEqual(['worker_contact:read', 'worker_pii:read', 'worker_document:read', 'match:read']);
  });

  it('cells = null / undefined → tudo legível (engine não decidiu, D113); [] → nada', () => {
    expect(workerContainerReadsOf(null)).toEqual({ contact: true, dossier: true, documents: true, encuadres: true, patientIdentity: true });
    expect(workerContainerReadsOf(undefined)).toEqual(ALL_WORKER_CONTAINERS_READABLE);
    expect(workerContainerReadsOf([])).toEqual({ contact: false, dossier: false, documents: false, encuadres: false, patientIdentity: false });
  ...
  it('cada célula abre SÓ o seu container; patient_identity:read é a do OUTRO titular', () => {
    expect(workerContainerReadsOf(['worker_pii:read'])).toMatchObject({ dossier: true, contact: false, documents: false, encuadres: false, patientIdentity: false });
  ...
  it('marcador: undefined quando nada foi redigido (resposta byte a byte a de antes); senão só os ocultos, sempre true', () => {
    ...
    expect(workerRedactionMarker(workerContainerReadsOf(['worker_contact:read']))).toEqual({ dossier: true, documents: true, encuadres: true });
  });
