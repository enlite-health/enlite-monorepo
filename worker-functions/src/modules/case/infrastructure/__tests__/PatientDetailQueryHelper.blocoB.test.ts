...
describe('fetchPatientDetail — bloco B', () => {
  it('seleciona as colunas novas (SQL) e mapeia; a nota sai crua daqui (a redação é do ponto único)', async () => {
    const { pool: p, query } = pool(
      ...
      { id: 'a1', address_type: 'primary', address_formatted: 'Rua X', address_raw: null, complement: null, display_order: 1, lat: '-34.6', lng: '-58.4',
    ...
    expect(out?.addresses[0]).toMatchObject({ neighborhood: 'Palermo', logisticsCorridor: 'Norte', accessNotes: 'timbre 2', country: 'AR', lat: -34.6, lng: -58.4 });
  });
  ...
  it('linha sem os campos novos → defaults (DONE, null, [], []) e endereço com nulls', async () => {
    const { pool: p } = pool(
      ...
      { id: 'a2', address_type: 'secondary', address_formatted: null, address_raw: 'cru', display_order: 2, lat: null, lng: null },
    );
    ...
    expect(out?.addresses[0]).toMatchObject({ neighborhood: null, logisticsCorridor: null, accessNotes: null, country: null, complement: null });
  });
});
