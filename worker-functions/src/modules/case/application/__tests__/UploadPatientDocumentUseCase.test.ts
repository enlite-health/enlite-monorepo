...
function makeDeps(overrides: { insertThrows?: boolean; deleteThrows?: boolean; orphanRecordThrows?: boolean } = {}) {
  const storage = {
  ...
  return { storage, repo, enc, orphanRepo };
}

/** Fábrica que sempre devolve o MESMO mock (item 1 da revisão do PR-4). */
...
describe('UploadPatientDocumentUseCase (spec 018 PR-4, D329)', () => {
  ...
  it('transação falha: apaga o objeto (best-effort) e relança — sem falha de delete, NÃO vira órfão', async () => {
  ...
  it('transação falha E a limpeza também falha — registra órfão DOCUMENTS/UPLOAD_FAILED e relança o erro ORIGINAL (achado item 4: antes era silencioso)', async () => {
  ...
  it('SEM GCS_PATIENT_DOCUMENTS_BUCKET: construir o use case NÃO lança (fábrica é preguiçosa) — só execute() lança', async () => {
    delete process.env.GCS_PATIENT_DOCUMENTS_BUCKET;
    // eslint-disable-next-line no-new -- prova do achado item 1 da revisão: boot não cai sem env.
    const uc = new UploadPatientDocumentUseCase();
    await expect(
      uc.execute({ patientId: PID, buffer: Buffer.from('%PDF'), contentType: 'application/pdf', documentType: 'image_consent', actorUid: 'uid-1' }),
    ).rejects.toThrow('GCS_PATIENT_DOCUMENTS_BUCKET não configurado');
  });
});
