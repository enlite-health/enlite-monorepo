 ...
 *
 * Um objeto entra na fila quando `PatientPhotoStorage` lançou ao apagar (falha real do GCS — 404
 * já é tratado como sucesso pelo storage, nunca vira órfão). O retry decifra o caminho (KMS),
 * tenta apagar de novo no bucket certo e só remove a linha da fila em caso de sucesso.
 *
 * `PatientDocumentStorage` foi REMOVIDO por completo (fix/018-remover-documentos-consentimento).
 * Linha antiga com `bucket='DOCUMENTS'` (se sobrar alguma na stage) não tem mais storage para
 * reprocessar — fica `stillFailing` para sempre, sem crash (ver `retryOne`).
 *
...
export class PatientPhotoOrphanRetryService {
  ...
  private async retryOne(row: PatientPhotoOrphanRow, client: PoolClient): Promise<boolean> {
    if (row.bucket !== 'PHOTOS') {
      // `patient_documents`/`PatientDocumentStorage` foram removidos — uma linha antiga com
      // `bucket='DOCUMENTS'` não tem mais storage para reprocessar. Nunca lança, só conta como
      // falha (mesma semântica de um erro de GCS de verdade).
      logger.warn({ orphanId: row.id, bucket: row.bucket }, '[PatientPhotoOrphanRetryService] bucket sem storage (documentos removidos) — não retentado');
      return false;
    }
    try {
      const objectPath = await this.enc.decrypt(row.object_path_encrypted);
      await this.photoStorage().delete(objectPath);
