...
export class CapabilityRegistry {
  ...
  private buildEntries(): CapabilityEntry[] {
    const { profileGet, documentsList, vacanciesList, interviewGet } = this.deps;
    return [
      {
        name: profileGet.constructor.name === 'WorkerProfileGetCapability'
          ? 'worker.profile.get'
          : (profileGet.constructor as { NAME?: string }).NAME ?? 'worker.profile.get',
        description: (profileGet.constructor as { DESCRIPTION?: string }).DESCRIPTION
          ?? 'Get worker profile.',
        inputShape: (profileGet.constructor as { INPUT_SHAPE?: Record<string, unknown> }).INPUT_SHAPE ?? {},
        execute: (args) => profileGet.execute(args),
      },
      {
        name: (documentsList.constructor as { NAME?: string }).NAME ?? 'worker.documents.list',
        description: (documentsList.constructor as { DESCRIPTION?: string }).DESCRIPTION
          ?? 'List worker documents.',
        inputShape: (documentsList.constructor as { INPUT_SHAPE?: Record<string, unknown> }).INPUT_SHAPE ?? {},
        execute: (args) => documentsList.execute(args),
      },
      {
        name: (vacanciesList.constructor as { NAME?: string }).NAME ?? 'worker.vacancies.list',
        description: (vacanciesList.constructor as { DESCRIPTION?: string }).DESCRIPTION
          ?? 'List worker vacancies.',
        inputShape: (vacanciesList.constructor as { INPUT_SHAPE?: Record<string, unknown> }).INPUT_SHAPE ?? {},
        execute: (args) => vacanciesList.execute(args),
      },
      {
        name: (interviewGet.constructor as { NAME?: string }).NAME ?? 'worker.interview.get',
        description: (interviewGet.constructor as { DESCRIPTION?: string }).DESCRIPTION
          ?? 'Get worker interview.',
        inputShape: (interviewGet.constructor as { INPUT_SHAPE?: Record<string, unknown> }).INPUT_SHAPE ?? {},
  ...
  private registerOne(
    ...
    const registerTool = server.registerTool.bind(server) as (name: string, config: Record<string, unknown>, cb: (args: unknown) => Promise<unknown>) => void;
    registerTool(
      entry.name,
      {
        description: entry.description,
        inputSchema: entry.inputShape,
      },
      async (args: unknown) => {
