 ...
 * `(patient_id, service_type, service_date)` é o índice único parcial no banco (migration 445),
 * não uma checagem no código: `insert` PODE lançar por violação de constraint, e é o CHAMADOR (F3,
 * fora do escopo desta porta) quem decide o que fazer com isso.
 */
...
export interface IAxonicoLancamentoRepository {
  /**
   * Busca a tentativa `status='enviado'` já gravada para a tripla `(patientId, serviceType,
   * serviceDate)` — a mesma tripla que o índice único parcial da migration 445 protege. `null` =
   * nenhum lançamento bem-sucedido ainda para esta tripla.
   */
  findExisting(
    patientId: string,
    serviceType: EnliteServiceType,
   ...
   * `status='enviado'` para a mesma tripla de uma tentativa `enviado` já existente ESTOURA por
   * violação do índice único parcial (`uq_axonico_lancamento_dedupe`), propositalmente: é essa
   * violação que prova, em teste, que o dedupe local é real.
   */
