...
const ROTAS: ScannedRoute[] = [
  { method: 'GET', path: '/api/admin/users', cell: { resource: 'user_management', action: 'read' } },
...
describe('GET /.well-known/permissions/routes', () => {
  ...
  it('lista só as rotas governadas, com célula e status', async () => {
    ...
    expect(res.body.totalRoutes).toBe(4);
    expect(res.body.governedRoutes).toEqual([
      { method: 'GET', path: '/api/admin/users', cell: 'user_management:read', status: 'declared' },
      { method: 'GET', path: '/api/admin/auth/profile', cell: null, status: 'exempt' },
      { method: 'POST', path: '/api/admin/coisa-nova', cell: null, status: 'undeclared' },
    ]);
  });
  ...
  it('não expõe dado de pessoa — só método, caminho e célula', async () => {
    const res = await request(appCom(passa)).get('/.well-known/permissions/routes').expect(200);
    expect(Object.keys(res.body.governedRoutes[0]).sort()).toEqual(['cell', 'method', 'path', 'status']);
  });
});
