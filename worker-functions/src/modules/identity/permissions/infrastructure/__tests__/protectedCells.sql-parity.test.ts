 ...
 * migration 296 (a recusa real, no banco). Este teste compara os CONJUNTOS —
...
describe('PROTECTED_CELLS × migration 296', () => {
  ...
  function arrayDoWarning(): string[] {
    ...
    if (!m) throw new Error('ARRAY do WARNING não encontrado na 296');
    return m[1].split(',').map((s) => s.trim().replace(/^'|'$/g, '')).filter(Boolean).sort();
  }

  it('o ARRAY do WARNING da 296 é EXATAMENTE PROTECTED_CELLS', () => {
