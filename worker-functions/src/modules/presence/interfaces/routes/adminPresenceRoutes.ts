...
/**
 * Rota de presença (heartbeat) — change 022-ux-mencao-e-notificacao, Rodada 2. Movida de
 * `@modules/identity` para módulo próprio `@modules/presence` (22/09/2026). Montada sob
 * `/api/admin`. Molde: `adminNotificationRoutes.ts` — mesma família `admin.users`, mesma CLASSE de
 * célula "own_*": o próprio staff sempre pode operar o PRÓPRIO estado, nasce concedida a TODO
 * grupo ativo (`migrations/466_grant_own_presence_all_staff.sql`, mesma regra de D-07/
 * `own_notifications`), nunca 0 grupos como `patient_conversation`/`staff_directory`.
 *
