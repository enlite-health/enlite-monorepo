-- 309 — remove o teto de `Tipo de Dispositivo` no CRU, e o motivo é uma contradição minha
--
-- ── O que aconteceu, em ordem, no mesmo dia ────────────────────────────────
-- A migration **306** deu teto 5 a `Tipo de Dispositivo` em `patient_source_labels`, com esta
...
--
-- A migration **307**, minutos depois, tornou o catálogo uma TABELA editável sem deploy —
...
-- a premissa da 306 é falsa e o cru trunca em silêncio** — na tabela cuja razão de existir é
-- impedir perda silenciosa (F32/F33).
--
-- Pior: o `5` ficou chumbado em DOIS lugares — o `CHECK` da 306 e
...
-- sozinho quando o catálogo muda. É o que a 306 tentou obter com um número e não conseguiu.
--
...
--
-- Rollback: restaurar o CHECK da 306. ⚠️ Falha se algum paciente já tiver 6+ dispositivos no
...
COMMENT ON CONSTRAINT patient_source_labels_ceiling_por_campo ON patient_source_labels IS
  ...
  'primeiro tipo novo (migration 309).';
