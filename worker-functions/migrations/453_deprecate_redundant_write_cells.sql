-- 453 — deprecia as 23 células `<recurso>:write` legadas que a 435 splitou em create/update
--
...
-- 20/09/2026): das 25 células `:write` vivas, 23 são hoje 100% REDUNDANTES — TODO grupo que tem
-- a `write` de um desses recursos também tem a `create` E a `update` do MESMO recurso (zero
-- órfãos: nenhum grupo perde acesso ao marcar a `write` como depreciada, porque `create`+`update`
-- já cobrem o que a `write` concedia). As outras 2 — `permission_management:write` e
-- `upload:write` — NÃO entram: o catálogo não tem `create`/`update` para esses dois recursos
-- (nunca foram splitados), então depreciar a `write` deles tiraria acesso real de `Acesso
-- Master`, `Super Admin` e `Recrutador`. Por isso a lista abaixo é NOMEADA, não
-- `WHERE action = 'write'` genérico — um filtro genérico pegaria os 2 excluídos.
--
...
--
-- O QUE FAZ: UPDATE de `deprecated_at = now()` nas 23 linhas `<recurso>:write` nomeadas —
...
DECLARE
  ...
  v_n INT;
BEGIN
  -- Guarda dura: `permission_management` e `upload` NUNCA podem estar nesta lista — eles não
  -- têm create/update no catálogo, e depreciar a write deles tira acesso real. Se algum dia
  -- alguém editar o array acima e incluir um dos dois por engano, a migration aborta.
  IF v_resources && ARRAY['permission_management', 'upload']::TEXT[] THEN
    RAISE EXCEPTION USING ERRCODE = '23514',
      MESSAGE = '[453] permission_management/upload não têm create/update — não podem entrar nesta lista';
  END IF;
  ...
  IF to_regclass('iam.permissions') IS NOT NULL THEN
    ...
    RAISE NOTICE '[453] iam.permissions: % células :write marcadas deprecated_at agora (das 23 nomeadas)', v_n;
  END IF;
