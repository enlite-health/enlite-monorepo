 ...
 * pessoal: resposta é `{ uid, displayName }[]`, nunca `email` nem `role` (D-06).
 *
...
describe('Diretório de staff (spec 022, Bloco 1) — HTTP real, Postgres real, engine ABAC ligado', () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `INSERT INTO users (firebase_uid, email, display_name, role, status, tenant_id) VALUES
         ($1, 'e022-staffdir-com-celula@e2e.local', 'Staffdir QA ComCelula', 'admin', 'ACTIVE', $4),
         ($2, 'e022-staffdir-sem-celula@e2e.local', 'Staffdir QA SemCelula', 'admin', 'ACTIVE', $4),
         ($3, 'e022-staffdir-inativo@e2e.local', 'Staffdir QA Inativo', 'admin', 'DEACTIVATED', $4)`,
      [U.comCelula, U.semCelula, U.inativo, TENANT_E2E],
    );
  ...
  it('2. busca parcial por nome — devolve staff ATIVO', async () => {
    const res = await chamar('/api/admin/staff-directory?q=Staffdir', U.comCelula);
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);

    const linha = res.body.data.find((e: { uid: string }) => e.uid === U.comCelula);
    expect(linha).toBeDefined();
    expect(linha.displayName).toBe('Staffdir QA ComCelula');
  });
  ...
  it('4. resposta NUNCA contém email nem role', async () => {
    const res = await chamar('/api/admin/staff-directory?q=Staffdir', U.comCelula);
    expect(res.status).toBe(200);
