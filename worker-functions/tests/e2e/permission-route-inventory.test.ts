  ...
  it('as famílias já viradas estão declaradas, rota a rota, com a célula certa', () => {
    ...
    expect(declaradas.map((r) => `${r.method} ${r.path} → ${(r.cells ?? [r.cell]).filter(Boolean).join('+')}`).sort()).toEqual(
      [
        ...
        'POST /api/admin/workers/sync-talentum → talentum:create',
        ...
        'POST /api/admin/vacancies/sync-talentum → talentum:create',
        ...
        'POST /api/admin/recruitment/calculate-reemplazos → recruitment:create',
        ...
        'POST /api/admin/template-drafts/validar → messaging:create',
