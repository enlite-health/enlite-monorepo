...
describe('inventário de rotas governadas (app real de pé)', () => {
  ...
  it('as famílias já viradas estão declaradas, rota a rota, com a célula certa', () => {
    ...
    expect(declaradas.map((r) => `${r.method} ${r.path} → ${(r.cells ?? [r.cell]).filter(Boolean).join('+')}`).sort()).toEqual(
      [
        ...
        'PATCH /api/admin/patient-chat-roles/:code → patient:update',
        ...
        'PATCH /api/admin/patients/:id/clinical → patient_clinical:update',
        'PATCH /api/admin/patients/:id/coverage → patient_coverage:update',
        // Escrita por linha (spec 018, PR-1, ADR-1; contracts/support-network.md).
        'PATCH /api/admin/patients/:id/coverage-emergency-contacts/:cid → patient_coverage:update',
        // Contatos externos sem vínculo familiar (spec 018, PR-2, `lex` #4).
        'PATCH /api/admin/patients/:id/external-contacts/:xid → patient_family:update',
        'PATCH /api/admin/patients/:id/general → patient_identity:update',
        // Escrita por linha (spec 018, PR-5, US-11) — célula NOVA `patient_care_team:write`.
        'PATCH /api/admin/patients/:id/professionals/:pid → patient_care_team:update',
        'PATCH /api/admin/patients/:id/responsibles/:rid → patient_family:update',
        'PATCH /api/admin/patients/:id/service → patient_services:update',
        'PATCH /api/admin/patients/:id/test-flag → patient:update',
        'POST /api/admin/patient-chat-roles → patient:create',
        ...
        'POST /api/admin/patients/:id/responsibles → patient_family:create',
        'POST /api/admin/patients/:id/responsibles/:rid/deactivate → patient_family:update',
        'POST /api/admin/patients/:patientId/addresses → patient_address:create',
        'PUT /api/admin/patients/:id/chat-ids → patient_chat:update',
        ...
        'POST /api/admin/workers/:id/documents/ingest-from-url → worker_document:create',
        'POST /api/admin/workers/:id/documents/save → worker_document:create',
        'POST /api/admin/workers/:id/documents/upload-url → worker_document:create',
        ...
        'DELETE /api/admin/vacancies/:vacancyId/workers/:workerId/contact-notes/:noteId → funnel:update',
        ...
        'POST /api/admin/interview-slots/:slotId/book → interview:update',
        'POST /api/admin/vacancies → vacancy:create',
        'POST /api/admin/vacancies/:id/generate-ai-content → vacancy:update',
        ...
        'POST /api/admin/vacancies/sync-talentum → talentum:create+talentum:update',
        'PUT /api/admin/encuadres/:id/move → funnel:update',
        'PUT /api/admin/encuadres/:id/result → funnel:update',
        'PUT /api/admin/vacancies/:id → vacancy:update',
        'PUT /api/admin/vacancies/:id/meet-links → vacancy:update',
        'PUT /api/admin/vacancies/:id/talentum-description → talentum:update',
        ...
        'POST /api/admin/recruitment/calculate-reemplazos → recruitment:create+recruitment:update',
        ...
        'POST /api/admin/messaging/templates → messaging:create',
        'PUT /api/admin/messaging/templates/:slug → messaging:update',
        ...
        'PUT /api/workers/:id/status → worker:update',
        'PUT /api/workers/:id/occupation → worker:update',
        'GET /api/workers/docs-expiring → worker_document:read',
        'PUT /api/workers/:id/doc-expiry → worker_document:update',
        ...
        'DELETE /api/admin/template-drafts/:id → messaging:update',
        ...
        'PATCH /api/admin/patients/:id/contracted-services/:sid → patient_services:update',
        'PATCH /api/admin/patients/:id/contracted-services/:sid/providers/:pid → patient_services:update',
        'PATCH /api/admin/patients/:id/diagnoses/:did → patient_clinical:update',
        'PATCH /api/admin/patients/:patientId/addresses/:addressId → patient_address:update',
        'POST /api/admin/catalogs/insurance-providers → patient:create',
        'POST /api/admin/map/corridor → patient_address:read',
        'POST /api/admin/patients/:id/contracted-services → patient_services:create',
        'POST /api/admin/patients/:id/contracted-services/:sid/providers → patient_services:update',
        'POST /api/admin/patients/:id/diagnoses → patient_clinical:create',
        'POST /api/admin/patients/map → patient_address:read',
        'POST /api/admin/template-drafts → messaging:create',
        'POST /api/admin/template-drafts/:id/duplicate → messaging:create',
        'POST /api/admin/template-drafts/:id/submit → messaging:update',
        'POST /api/admin/template-drafts/validar → messaging:create+messaging:update',
        'POST /api/admin/workers/:workerId/presentation-invite → messaging:send',
        'POST /api/admin/workers/map → worker_address:read',
        'PUT /api/admin/funnel-stage-messages/:stage → messaging:update',
        'PUT /api/admin/presentation-invite/settings → messaging:update',
        'PUT /api/admin/template-drafts/:id → messaging:update',
        ...
        'POST /api/admin/patients/:id/therapeutic-projects → patient_therapeutic_project:create',
        'POST /api/admin/patients/:id/therapeutic-projects/:vid/annul → patient_therapeutic_project:update',
        'GET /api/admin/therapeutic-catalogs/specific-objectives → catalog_therapeutic_objectives:read',
        'POST /api/admin/therapeutic-catalogs/specific-objectives → catalog_therapeutic_objectives:create',
        'PATCH /api/admin/therapeutic-catalogs/specific-objectives/:itemId → catalog_therapeutic_objectives:update',
        'GET /api/admin/therapeutic-catalogs/activities → catalog_therapeutic_activities:read',
        'POST /api/admin/therapeutic-catalogs/activities → catalog_therapeutic_activities:create',
        'PATCH /api/admin/therapeutic-catalogs/activities/:itemId → catalog_therapeutic_activities:update',
  ...
  it('as isentas são as NOVE decididas, e nenhuma a mais', () => {
    ...
    // As TRÊS `/v1/me/simulation*` entraram na spec 026 (D407, F2): mesma marca
    // (`exemptHandler` em `meSimulationRoute.ts`), mesmo motivo do `/v1/me/authz`
