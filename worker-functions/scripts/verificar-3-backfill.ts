/**
 * verificar-3-backfill.ts — prova o MECANISMO do backfill da 2.4 sem tocar em produção.
 *
 * O problema que este script resolve: o banco local é de teste e não tem os 263 pacientes
 * reais, então `backfill-3-cobertura.ts` contra o local mede `OBTIDO=0` e recusa —
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const a of alvos) {
      await cli.query(
        `INSERT INTO patients (first_name,last_name,country,is_test,clickup_task_id)
         VALUES ('Sintetico','FIXTURE-BF3','AR',true,$1)`, [a.taskId]);
    }
    const conta = async () => Number((await cli.query(
      `SELECT count(*) n FROM patient_insurance_verified l
         JOIN patients p ON p.id = l.patient_id
        WHERE p.last_name='FIXTURE-BF3' AND l.source='backfill-3'`)).rows[0].n);
    ...
    const saida = execFileSync('npx', ['ts-node','-r','tsconfig-paths/register',
      'scripts/backfill-3-cobertura.ts','--executar','--esperado',String(QUANTOS)],
    ...
    const del = await cli.query(`DELETE FROM patient_insurance_verified WHERE source='backfill-3'`);
    ...
    const derivado = Number((await cli.query(
      `SELECT count(*) n FROM patients WHERE last_name='FIXTURE-BF3' AND insurance_verified IS NOT NULL`)).rows[0].n);
    ok('o ESCALAR nunca foi tocado', derivado === 0, '`patients.insurance_verified` não é escrita por este backfill');
  } finally {
    await cli.query(`DELETE FROM patients WHERE last_name='FIXTURE-BF3'`);
    const sobrou = (await cli.query<{ n: string }>(`SELECT count(*) n FROM patients WHERE last_name='FIXTURE-BF3'`)).rows[0].n;
